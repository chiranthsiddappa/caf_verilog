...
class TestCpxMultiply(TestCase):
    ...
    def test_cpx_multiply_x_real(self):
        cpx_multiply = CpxMultiply(self.x, self.x, x_i_bits=8, output_dir=self.tmpdir.name)
        npt.assert_almost_equal(cpx_multiply.x_quant, self.test_x)

    def test_cpx_multiply_x_imag(self):
        xj = self.x * 1j
        x = self.x + xj
        cpx_multiply = CpxMultiply(x, x, x_q_bits=8, output_dir=self.tmpdir.name)
        npt.assert_almost_equal(cpx_multiply.x_quant.imag, self.test_x)

    def test_cpx_multiply_y_real(self):
        cpx_multiply = CpxMultiply(self.x, self.x, y_i_bits=8, output_dir=self.tmpdir.name)
        npt.assert_almost_equal(cpx_multiply.y_quant, self.test_x)

    def test_cpx_multiply_y_imag(self):
        xj = self.x * 1j
        x = self.x + xj
        cpx_multiply = CpxMultiply(x, x, y_q_bits=8, output_dir=self.tmpdir.name)
    ...
    def test_cpx_multiply_gen_quantized_output(self):
        ...
        cpx_multiply = CpxMultiply(x, y, output_dir=self.tmpdir.name)
